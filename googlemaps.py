...
class GoogleMaps(object):
    ...
    **Directions**: turn-by-turn directions, distance, time, etc. from 
    ...
    def directions(self, origin, destination, mode='driving', waypoint=['']):
        """
        ...
        """
        if waypoint[0] == '':
            params = 'origin=' +origin+ '&destination=' +destination+ '&mode=' +mode+ '&sensor=false'
        else:
            params = 'origin=' +origin+ '&destination=' +destination+ '&mode=' +mode+ '&waypoints=' +str(waypoint)+ '&sensor=false'
        
        #params.update(kwargs)

        response = fetch_json(self._DIRECTIONS_QUERY_URL, params)
        #status_code = response['Status']['code']
        #if status_code != STATUS_OK:
         #   raise GoogleMapsError(status_code, url=url, response=response)
        return response
