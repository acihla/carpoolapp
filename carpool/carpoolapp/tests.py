...
class SignupTest(testLib.RestTestCase):
    ...
    def testSignup8(self):
        respData = self.makeRequest("/signup",method ="POST",data ={'firstname':'AJ','lastname':'Cihla','email':'alex.cihla@yahoo.com','dob':'04/17/1992','sex':'male','password':'password','cellphone':'5104593078','driver':False})
        print("signupTest8")
        self.assertResponse(respData, testLib.RestTestCase.SUCCESS)
...
class LoginTest(testLib.RestTestCase):
    ...
    def testLogin1(self):
        respData = self.makeRequest("/login",method="POST",data = {'email' :'alex.chila@berkeley.edu','password':'password'})
        print("testLogin1")
        self.assertResponse(respData, testLib.RestTestCase.SUCCESS)
    #check invalid email
    def testLogin2(self):
